...
<nav :class="intro ? '!translate-y-0' : ''"
    ...
    if ($catMode) :
        ...
        <button @click="openForm = true" type="button" aria-label="open add to cart form"
                class="bg-primary px-5 py-3 group/add flex gap-x-2 justify-center hover:brightness-125 transition-all text-center rounded-lg text-white">
    ...
    <div
        ...
        <div :class="openForm ? 'delay-200 !translate-y-0' : 'translate-y-full'"
            ...
            <button @click="openForm = false" aria-label="close menu"
                    class=" text-black mb-2 absolute -top-4 start-3 bg-gray-100 border border-gray-200 p-2 rounded-sm">
                <?php
                $args = array(
                    'size' => '20',
                    'class' => '',
