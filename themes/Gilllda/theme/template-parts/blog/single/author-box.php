...
?>
<section class="xl:w-3/4 border bg-gray-50/50 flex gap-x-5 gap-y-2 max-lg:flex-col max-lg:items-center  border-black/30 rounded-lg min-h-40 p-6">
        ...
        <?php else :
            $args = array(
                'size' => 100,
                'class' => 'bg-white border rounded-full my-auto p-2 border-gray-400'
            );
    ...
    <div class="flex-1 flex max-lg:flex-col gap-5 items-center justify-between">
       <div class="flex flex-col lg:w-2/3 max-lg:items-center gap-y-2">
        ...
        <a aria-label="like to author page" class="bg-black text-white lg:text-black lg:bg-white flex text-nowrap max-lg:w-full justify-center items-center lg:hover:bg-black transition-all hover:text-white gap-x-2 rounded-lg px-4 py-3 border border-black" href="<?=  get_author_posts_url($author_id); ?>">
