...
if ($show) :
    ...
    <div x-data="popupComponent()"
        ...
        <div class="absolute before:absolute scale-[102%] top-1/2 inset-x-1/2 translate-x-1/2 w-11/12 lg:w-[600px] -translate-y-1/2 aspect-3/4 md:aspect-video overflow-hidden rounded-lg before:rounded-xl before:bg-white before:size-full before:scale-[97.5%] lg:after:w-2/3 after:w-full after:absolute after:start-1/2 after:origin-right after:top-1/2 after:animate-spin after:h-56 after:-translate-y-1/2 after:bg-gradient-to-b after:from-secondary animate-duration-4000"
             :class="popup ? 'delay-500 opacity-100' : 'opacity-0'"></div>
        <div class="relative max-w-11/12 lg:max-w-[595px] group translate-y-1/2 aspect-3/4 border border-white/20 md:aspect-video after:ease-linear flex flex-col rounded-md ease-out overflow-hidden transition-all duration-500" :class="popup ? '!translate-y-0' : ''">
            <button @click="closePopup()" aria-label="close popup"
                    class="text-white absolute top-3 start-3 bg-secondary border border-gray-300 shadow p-1.5 cursor-pointer hover:brightness-110 z-1 rounded-md">
            ...
            <picture>
                ...
                <img class="lg:max-w-[600px] group-hover:scale-110 transition-all duration-300 object-cover aspect-3/4 md:aspect-video" src="<?= esc_url($popUp['image']['desktop']['url'] ?? ''); ?>" alt="<?= esc_attr($popUp['image']['desktop']['title'] ?? ''); ?>">
            </picture>
            <a class="absolute bottom-3 inset-x-3 border font-bold border-white/40 py-4 shadow text-center rounded-md text-lg text-white bg-secondary hover:brightness-125 transition-all" href="<?= esc_url($popUp['link']['url'] ?? '#'); ?>"><?= esc_html($popUp['link']['title'] ?? ''); ?></a>
        </div>
    </div>
    <script>
