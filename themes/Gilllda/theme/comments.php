...
<div x-data="commentsSystem()" @click="handleClicks($event)">
        ...
        <div class="container grid relative pb-5 h-fit lg:grid-cols-12 gap-4 items-start my-8 py-10 p-5">
            ...
            <div id="comment-form-wrapper"
                ...
                <div x-show="isReplying"
                     ...
                     style="display: none;"
                     class="mb-4 bg-primary/10 border border-primary/15 p-3 rounded-xl flex items-center justify-between">
                    <div class="text-sm text-primary flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/>
                        </svg>
                        پاسخ به: <strong class="font-black" x-text="replyToName"></strong>
                    </div>
            ...
            <div class="lg:col-span-7 xl:col-span-8 max-lg:order-2 overflow-hidden">
                <?php if (have_comments()) : ?>
                    <div id="bluebox-comments-container">
                        <ol class="lg:space-y-4 max-lg:mt-6 pl-4 max-lg:flex max-lg:max-w-[100vw] max-lg:gap-4 max-lg:overflow-x-auto max-lg:snap-x max-lg:snap-mandatory pb-4" id="bluebox-comments-list">
                            <?php
                            global $wp_query;
    ...
    <div x-show="isModalOpen"
        ...
        <div x-show="isModalOpen"
             ...
             x-transition:leave-end="opacity-0 translate-y-full lg:translate-y-10 lg:scale-95"
             class="bg-white w-full lg:w-[500px] max-h-[85vh] lg:max-h-[70vh] rounded-t-3xl lg:rounded-3xl relative z-10 flex flex-col shadow-2xl">
            ...
            <header class="flex items-center justify-between p-5 border-b border-gray-100 shrink-0">
                <h3 class="font-black text-gray-800 text-lg flex items-center gap-2">
                    پاسخ‌ها به <span class="text-primary" x-text="modalAuthorName"></span>
                </h3>
                <button @click="closeModal()" class="bg-gray-100 hover:bg-red-50 text-gray-500 hover:text-red-500 p-2 rounded-full transition-colors">
        ...
        Alpine.data('commentsSystem', () => ({
            ...
            handleClicks(e) {
                ...
                if (viewRepliesBtn) {
                    e.preventDefault();
                    this.openModal(
                        viewRepliesBtn.getAttribute('data-comment-id'),
                        viewRepliesBtn.getAttribute('data-author')
                    );
                    return;
                }
                ...
                if (replyBtn) {
                    ...
                    this.initiateReply(
                        ...
                        replyBtn.getAttribute('data-author')
                    );
                }
            },

            openModal(commentId, authorName) {
                this.modalAuthorName = authorName;
                this.repliesHtml = '';
                this.isLoading = true;
                ...
                fetch(ajaxUrl, { method: 'POST', body: formData })
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            this.repliesHtml = data.data;
                        } else {
                            this.repliesHtml = '<p class="text-center text-sm text-red-500 py-5">خطا در دریافت پاسخ‌ها.</p>';
                        }
                    })
                    .catch(err => {
                        this.isLoading = false;
            ...
            initiateReply(commentId, authorName) {
